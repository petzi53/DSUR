...
rv <- reactiveValues(data = as_tibble(rbeta(10000,5,5)),
                     caption = "Normal distribution")

observeEvent(input$normal, {
            rv$data <- as_tibble(rbeta(10000,5,5))
            rv$caption <- "Normal distribution"})
observeEvent(input$right,  {
            rv$data <- as_tibble(rbeta(10000,2,5))
            rv$caption <- "Positivey or right-skewed distribution"})
observeEvent(input$left,   {
            rv$data <- as_tibble(rbeta(10000,5,2))
            rv$caption <- "Negatively or left-skewed distribution"})

output$hist1 <- renderPlot({ 
    ggplot(rv$data, aes(value)) +
            geom_histogram(aes(y = ..density..), 
                           alpha = 0.25, fill = "blue", binwidth = .025) +
            stat_function(fun = dnorm, 
                          args = c(mean = mean(rv$data$value), 
                                   sd = sd(rv$data$value)), 
                          col = "tomato") +
        labs(
            title = rv$caption,
