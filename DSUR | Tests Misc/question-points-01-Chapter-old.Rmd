...
# --------------------------------------------------------------
source('~/Documents/Meine-Repos/DSUR/tutorial/www/check_test.R')
...
### Sometimes I use a different structure
...
Sometimes I added some R related information or even decided to depart from DSUR content. The last is especially true with `RStudio` which installation and GIUs I used instead of the standard R Interface or the program package `R Commander`. But I have noted this differences within a yellow box.
...
This chapters could be divided into two part: It beginns with an introduction to the research process: How to start with initial observations, generate theories and hypothesis to test the assumptions.
...
To check if you are conversant with these notions you could take the [multiple choice test of this chapter](https://studysites.uk.sagepub.com/dsur/study/mcq/Chapter%201/quiz.htm) taken from the student resources of the companion site. Additionally I have converted another [13 questions](#section-quiz01) from the part of the instructor resources.
...
## Data Input of a small data set

### Read data values from keyboard

To provide exercises, we need some data. 
...
**This function cannot be demonstrated, because it would need interactivity outside this interactive tutorial.**

```{r read-data-from-keyboard, eval=FALSE, echo=TRUE}
nff_scan <- as.vector(scan(file = ""))
```

### Assign values to a vector

Assume you have a list of 11 Facebook user with their number of friends.
108, 103, 252, 121, 93, 57, 40, 53, 22, 116, 98.

We want to assing this data set into a variable `nff`, which stands for "number of facebook friends".

How can you get these data into R?

+ Create a variable `nff` (use the assign operator)
+ Assign the above data to it (use the combine function)
+ Sort the vector (use the sort function)
+ display the content of the resulting vector

There are accumulative hints demonstrating each bullet point.

```{r assign-values-to-a-vector, exercise=TRUE}
...
```{r assign-values-to-a-vector-hint-1}
nff <- "Replace this string with the value(s) to assign"
```


```{r assign-values-to-a-vector-hint-2}
nff <- c(108, 103, 252, 121, 93, 57, 40, 53, 22, 116, 98)
```


```{r assign-values-to-a-vector-hint-3}
nff <- c(108, 103, 252, 121, 93, 57, 40, 53, 22, 116, 98)
nff <- sort(nff)
```


```{r assign-values-to-a-vector-solution}
nff <- c(108, 103, 252, 121, 93, 57, 40, 53, 22, 116, 98)
nff <- sort(nff)
nff
```
