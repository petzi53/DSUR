...
library(modeest)
# M <- mlv(d.normal$value, method = "naive", bw = 0.025)[[1]]
# M <- mlv(d.normal$value, method = "discrete")[[1]]
p.normal.line <- p.normal +
    geom_vline(
        xintercept = mlv("beta", 5, 5)[[1]],
        linetype = "dashed",
        color = "black",
        size = 1
        ) +
...
library(tidyverse)
my.norm <- NULL
my.mean1 <- sample(-10:50, 1)
my.sd1 <- sample(1:5, 1)
my.mean2 <- sample(-10:50, 1)
my.sd2 <- sample(1:5, 1)
my.norm$eins <- rnorm(10000, 48, 48)
my.norm$zwei <- rnorm(10000, 48, 12)
ggplot(as_tibble(my.norm), aes(eins)) +
  geom_histogram(bins = 100, alpha = 0.25, fill = "blue") +
  geom_histogram(aes(zwei), bins = 100, alpha = 0.25, fill = "red",)
```

```{r}
t.v <- rbeta(10000,10,3)
ggplot(as_tibble(t.v), aes(value)) +
  geom_histogram(bins = 100, alpha = 0.25, fill = "blue")
```
