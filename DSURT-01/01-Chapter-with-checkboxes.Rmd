...
pos.mode <- pos.mean <- pos.median <- 0
linetype <- NA
...
```{r my-dist-ui1, context="render"}
    fluidRow(
        ...
        column(4,
            checkboxGroupInput("measure", "Measure to show:",
                                 c("Mode" = "mode",
                                   "Mean" = "mean",
                                   "Median" = "median")
                              )
...
output$distPlot <- renderPlot({
    output$txt <- renderText({paste("Data1:", input$measure[1],
                                    "Date2:", input$measure[2],
                                    "Data3:", input$measure[3],
                                    "Type", typeof(input$measure),
                                    "Class", class(input$measure))})
    switch(input$type,
        norm    = {histo <- as_tibble(rbeta(10000,5,5))
                  caption <- "Normal distribution"
                  ifelse("mean" %in% input$measure,
                      {pos.mean <- mean(histo$value) 
                               linestyle <- "dotdash"}, linestyle <- NA)
                  ifelse("median" %in% input$measure,
                      {pos.median <- median(histo$value) 
                               linestyle <- "twodash"}, linestyle <- NA)
                  ifelse("mode" %in% input$measure,
                      {pos.mode <- mlv(histo$value, method = "discrete")[[1]] 
                               linestyle <- "dashed"}, linestyle <- NA)
          },
